...
public class ExceptionsLecture {
	...
	public static void main(String[] args) {
		...
		try {	// If an Exception occurs in this block of code, I will handle it with a catch block
		...
		} catch(ArrayIndexOutOfBoundsException e) {	// if this Exception happens in the preceding TRY block, I will handle it
			// When a catch is executed an Exception object is passed to it as a parameter
			//		in this example we called the Exception object 'e' -> 'exceptionObj' is better, more descriptive
			...
			System.out.println(e.getMessage());	//Display the system message for the Exception
//			e.printStackTrace();	// This will display the stack trace from main() down to the
									// method causing the Exception
									// in this case nothing is displayed because we are in main()
		}
		...
		try {
			...
			doSomethingDangerous();  // throws an ArrayIndexOutOfBoundsException which we will handle if it doesn't
		...
		try {
			...
			// generic header for Exceptions -> best practice BE SPECIFIC
		...
		try {
			double amountOwed = calculateHotelRoomCharges(nights, numberOfGuests);	// may throw and IllegalArgumentException
			System.out.println("Total owed for "+numberOfGuests+" guests for "+nights+" nights is $"+amountOwed);
		} catch(IllegalArgumentException e) {	//handle any IllegalArgumentException thrown in the try block
	...
	private static double calculateHotelRoomCharges(int nights, int numberOfGuests) {
		...
		*/
		if(nights < 1) {
			throw new IllegalArgumentException("Minimum number of nights is 1"); // Throw Exception
		} else if(numberOfGuests < 1) {
			throw new IllegalArgumentException("Minimum number of guests is 1"); // Throw Exception
		}
