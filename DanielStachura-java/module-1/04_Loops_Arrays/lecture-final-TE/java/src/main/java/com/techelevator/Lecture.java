...
public class Lecture {
    ...
    */
    public int returnFirstElement() {
        ...
        return portNumbers[0]; // first element number 0
    }
    ...
    */
    public int returnLastElement() {
        int[] portNumbers = { 80, 8080, 443 };
        return portNumbers[portNumbers.length-1]; // index of last is ALWAYS .length-1
    }

    /*
    4. Return the first element of the array from the parameters
    */
    //                                   parameter(s) may be arrays
    public int returnFirstElementOfParam(int[] passedInArray) {
        return passedInArray[0];  // use name of the parameter[0]
    }
...
//                   i=0  < 4   i=0,1,2,3,4(no loop)
        for (int i = 0; i < arrayToLoopThrough.length; i++) { // loop 3 times, i=4 at then end
// before next stmt counter=4
            counter++;
        }
