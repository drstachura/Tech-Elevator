 ...
 */
@FixMethodOrder(MethodSorters.NAME_ASCENDING) //OPTIONAL - List the tests in ascending order
...
public class LectureTest {	// Class to hold all the JUnit test methods in Lecture class
	...
	@Before	//do this before EVERY test - any set up of data that needs to be done for all tests
	public void setup() {
	...
	@After //do this AFTER every test
	public void teardown() {
		System.out.println("teardown");
	}	//method that holds all your processing after every test
																// the name cane be anything you want - descriptive
	...
	public void length_returns_the_number_of_characters_in_a_String() {	//this is the Method
		...
		String theString = "Java"; 			// Arrange - identify/define the input data for the test
		int length = theString.length(); 	// Act - run the method with the test data
	//		       expected 4, actual result
		Assert.assertEquals(4, length); 	// Assert - verify the result from the Act was the expected result
												// tests are a series of asserts
	}
	...
	public void startsWith_returns_true_if_a_string_starts_with_the_specified_characters() {
		 ...
		 * what went wrong */
		//Do not specify and expected result, it's obvious assertTrue expects true
