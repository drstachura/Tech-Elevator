...
public class ItalianPlayingCard extends PlayingCard {
	...
	public ItalianPlayingCard() 
	{
		...
		initializeMaps();	//delay initialization until at least one object is constructed
	...
	public ItalianPlayingCard(int value, String suit) {
		super(value, suit, suitMap.get(suit));{	// we are using the suitMap assuming it has already been initialized
												//
												//We don't actually initialize theMaps until the end of this CTOR
												//IF a default constructor was not run before this ctor, the Maps are not initialized
		...
		initializeMaps();						//this will work sometimes, but not very safe.  should initialize outside
												//of the class.  initialize with static variables
		}
