...
public class IntroToClassesCard {
	...
	//  access		  return   name(parameters)
		public static  void    main(String[] args) { //immediatley to the left of ( is function/method name
        ...
        */
		...
		PlayingCard johnsCard = new PlayingCard(0,"Hearts");

		System.out.println("is franksCard equal to johnsCard?" + (franksCard == johnsCard));

		// == compares the location of reference variables - objects are reference variables
		// so == can't be used to compare 2 objects(franksCard and johnsCard are objects)

			System.out.println("is franksCard equal to johnsCard using ==?" + (franksCard == johnsCard));

		//
		//if we want to be able tyo compare 2 objects the class MUST provide a method to do so
		// Java does not know how to comp[are 2 objects using .equals
		// if the PlayingCard class has a .equals(), it will be used to do the comparew
