...
// Tell Spring to automatically convert JSON dates to LocalDates
	...
	// 		we need to provide a way to have the JSON that goes between us and the server convert
	//		to the language specific data type
	@JsonDeserialize(using = LocalDateDeserializer.class) //Spring provided process to convert JSON string to LocalDate
	@JsonSerialize(using = LocalDateSerializer.class)     //Spring provided process to convert JSON
	...
	//Use a Spring provided process to convert a LocalDate to a String with the format we want
	//	when Spring takes an object and serializes into JSON for API calls
	//		we need/can tell it what format we want our LocalDate to be as a String
