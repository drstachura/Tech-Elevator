...
-- note that group is a reserved word - you should avoid using SQL reserved word in any names if you can
--      SQL language has a 'group by' try to avoid using 'group' in any names
CREATE TABLE interest_group (
	group_id serial,
	name varchar(40) NOT NULL,
	CONSTRAINT PK_interest_group PRIMARY KEY (group_id),
	CONSTRAINT UQ_interest_group_name UNIQUE (name) -- enforce uniqueness of the name even tho its not part of PK
);
--
-- in these examples, the FOREIGN KEY constraing is specified in the CREATE TABLE 
-- rather than an ALTER statement to add FOREIGN KEY 
--
-- When specifying the FOREIGN KEY contraint in the CREATE TABLE, the parent table must already have been created
--      the order of the table creates is critical (it matters)
--
-- Using ALTER statement to add FOREIGN KEY after all the tables are created, means they can be created in any order
--      more flexibility and fewer opportunities for errors if you add the FOREIGN KEY with ALTER rather than on the CREAT TABLE
