        ...
        while (menuSelection != 0) {
            ...
            try {
                if (menuSelection == 1) {
                    ...
                    // Make sure we got exactly 2 words separated by a ,
                    if( credentials.split(",").length == 2 ) {
                        // If we got properly formatted input, send it to the AuthenticationService login()
                        // the login() is returning a HTTP response which we are storing in a variable called response
                        //      the response is a Map, so use .get(key) to get the value in the Map
