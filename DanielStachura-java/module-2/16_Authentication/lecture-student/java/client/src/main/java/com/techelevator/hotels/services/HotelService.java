...
public class HotelService {
  ...
  private final String BASE_URL;  // server base URL, it will be passed when the service is instantiated
  ...
  public Reservation getReservation(int reservationId) throws HotelServiceException {
    ...
    try {
      ...
      // Call the server with (1) the base url + the reservations/{id} path   - BASE_URL + "reservations/" + reservationId
      //                      (2) type if HTTP request                        - HttpMethod.GET
      ...
      //                      (6) store it in our variable called reservation - reservation =
  ...
  private HttpEntity<Reservation> makeReservationEntity(Reservation reservation) {
    ...
    headers.setBearerAuth(AUTH_TOKEN);  // we need to set the authorization header to the JWT
