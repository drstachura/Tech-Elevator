...
public class App {
    ...
    public static void main(String[] args) {
        int     menuSelection;
        boolean keepLooping;  // Loop control variable
        int     hotelId = -1;

        ConsoleService consoleService = new ConsoleService();
        HotelService   hotelService   = new HotelService(API_BASE_URL);

        keepLooping = true;      // Set loop control variable to loop
        while (keepLooping) {
            ...
            if(menuSelection == 1) {
               ...
               keepLooping = false;   // Set loop control variable to end loop
            } else {
        ...
        consoleService.allDone();  // Call ConsoleService method to handle anything it needs to do when we are done
