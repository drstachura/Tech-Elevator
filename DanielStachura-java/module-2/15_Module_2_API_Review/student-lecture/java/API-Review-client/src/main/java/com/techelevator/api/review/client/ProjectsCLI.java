...
public class ProjectsCLI {
	...
	// newDepartment = departmentDAO.createDepartment(newDepartment)
		...
		// Define a RestTemplate object to call the API
	//	RestTemplate callApi = new RestTemplate();        ***DEFINED AT THE TOP

		// Because we need to do a HTTP POST:
		// 		We need header to define the content type we are sending - HttpHeaders
		//		We need to create a request with the headers and new department - HttpEntity
		// 		We need to send the the request (HttpEntity) to the POST
		HttpHeaders theHeaders = new HttpHeaders();							// Define request Headers
		theHeaders.setContentType(MediaType.APPLICATION_JSON);				// Set the contentType
	...
	//	RestTemplate callApi = new RestTemplate();	// Define a RestTemplate object to make and manage the API calls  *** DEFINED AT THE TOP

		// RestTemplate can return an array of objects automatically for us, so here is an array to hold the return values
		Department[] deptArray; // Hold the Department objects returned from the API call
		...
		// Call our method to list the departments with the data from the API call
		listDepartments(Arrays.asList(deptArray));	// need to convert the array we have to a List for the method
	}
