...
function scopeTest() {
  ...
  // using a variable that appears to have not yet been defined - we don't see it before this line of code
  console.log(`Baddy McBadBad contains: ${baddyMcBadBad}`)
  {
    // this variable lives inside this block and doesn't
    // exist outside of the block
    // variables defined inside a block are only available inside that block or blocks within it
 ...
 * 
 * .join() will creates a string from an array with separator specified 
 *                  (or , if no separators)
 * 
 * @param {*} name 
 * @param {*} age 
...
/**
 * Takes an array and, using anonymous functions, generates their sum.
 *
 ...
 * 
 * We want a SINGLE VALUE out of an array - This is a job for reduce()!
 * 
 ...
 *              receives a parameter which is an element in the array
 * 
...
function sumAllNumbers(numbersToSum) {
  // in this anon-func we will call the reducer sum since it will hold the sum of the array elements
 ...
 * the anon-func determines if the elements it is sent meets the conditions to be included in the new array
 *    it returns true if it does, false if it doesn't
 * 
 * .filter() will add the current element to the new array if anon-func returns true
 * 
...
function allDivisibleByThree(numbersToFilter) {
  return numbersToFilter.filter( (currElem) => {
                                                if (currElem % 3 == 0){ // is the currElem a multiple of 3
...
/* Alternate solutions
    ...
    return numbersToFilter.filter( (currElem) => {return !currElem % 3} ); // Uses truthy/falsy
                                                            3      % 3
                                                                  0 - false
                                                                 !0 - true
*/
 ...
 * 
 *  Create a new array containing the squares of the corresponding elements in another array
 * 
...
.map() adds the value returned from the anon-func to the new array
                                 ...
                                 "Lindsay", "Anthony","Lorenzo","Array McRayRay"])
{
  ...
  console.table(anArray.sort());  // sort function sorts an array and replace the original

  anArray.sort();                 // same as above
  console.table(anArray);

  anArray.reverse();              // reverse elements in an array and replace the original
  console.table(anArray);
}
