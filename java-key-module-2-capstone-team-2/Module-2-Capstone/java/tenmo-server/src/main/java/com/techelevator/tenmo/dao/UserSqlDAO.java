...
//        String sqlGetUserInfo = " SELECT user_id, username " +
//                " FROM users ";
//        SqlRowSet results = jdbcTemplate.queryForRowSet(sqlGetUserInfo);
//        while (results.next()) {
//            User userResult = mapToUser(results);
...
//    }
    ...
    public List<User> findAllWithoutHash(Long user_id, String username) {
        List<User> allUsers = new ArrayList<>();
        String sqlGetUserInfo = " SELECT user_id, username " +
                " FROM users ";
        SqlRowSet results = jdbcTemplate.queryForRowSet(sqlGetUserInfo);
    ...
    private User mapToUser(SqlRowSet results) {
        ...
        theUsers.setId(results.getLong("user_id"));
        theUsers.setUsername(results.getString("username"));
        return theUsers;
    }
}
